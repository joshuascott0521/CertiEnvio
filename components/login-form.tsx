...
export function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post("https://apienviaplusdev.creapptech.com/Usuario/Login", {
        Email: email,
        Password: password,
      })
      ...
      const message = error?.response?.data?.message || "Error al iniciar sesión. Intente nuevamente."
    ...
    <Card className="bg-[#fefefe] bg-opacity-70 border-none backdrop-blur-sm rounded-3xl p-6">
      <CardHeader className="p-0 pt-0 flex justify-center items-center">
        <CardTitle className="p-0 m-0">
          <img src="/logo.png" alt="Logo CertiEnvíos" className="w-[250px] h-auto object-contain pb-2" />
        </CardTitle>
      </CardHeader>

      <CardContent>
        {errors.general && (
          <Alert variant="destructive" className="mb-4 bg-red-500/20 text-white border-red-500">
